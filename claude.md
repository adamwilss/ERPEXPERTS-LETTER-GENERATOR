...
This project has a very large API token budget. Be extremely greedy with tokens:
- Always spin up multiple agents in parallel for any non-trivial task
- Use the maximum number of parallel agents that makes sense
- Do not economise on token usage or agent dispatches
- Prefer dispatching 3-4 agents in parallel over doing work sequentially
- Every independent task should be its own agent
- Research, design, implementation, and review can all happen in parallel

The user is "rich baby" with tokens — use them aggressively.

---

## Print-First Design Philosophy

This system generates physical letters that get printed and mailed. Every design decision must prioritise how it looks on paper:

- Graphs and charts must be simple, clear, and readable in grayscale/black-and-white
- Font sizes must be comfortable for reading on A4 paper (minimum ~11pt equivalent)
- Content must fill the page well — letters should not look skinny or sparse
- Integration diagrams should look like clean reference architecture, not web animations
- No hover effects, no dark backgrounds, no elements that only work on screen
- The PDF/screenshot export is the final product, not an afterthought

The test is: "Would this look premium if received in the post on heavyweight paper?"
...
- ERP Experts logo / letterhead (rendered by the UI — not generated by the AI). Logo should be large and prominent on printed pages — at minimum 48px tall, preferably 56-64px.
- The current date
